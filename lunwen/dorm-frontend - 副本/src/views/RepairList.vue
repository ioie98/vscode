    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="display: flex; justify-content: flex-start; align-items: center;">
        <div class="header-actions" style="display: flex; align-items: center; gap: 10px;">
          <el-checkbox v-if="userRole !== 1" v-model="showOnlyPending" label="只看待处理" style="margin-bottom: 10px;" @change="fetchData" border />
          <!-- <el-button icon="Refresh" circle style="margin-bottom: 10px;" @click="fetchData" /> -->
          <el-button v-if="userRole === 1" type="primary" icon="Plus" @click="handleCreate" style="margin-bottom: 10px;">我要报修</el-button>
        </div>
      </div>
        ...
        <el-table-column label="操作" width="220" fixed="right" align="center">
          <template #default="scope">
            ...
            <template v-if="userRole === 0">
              ...
              <!-- 状态1：显示撤回 (新增功能) -->
              <el-button v-if="scope.row.status === 1" size="small" type="info" @click="handleRevoke(scope.row)">撤回</el-button>
...
const handleRevoke = (row) => {
  ElMessageBox.confirm(`确定要撤回【${row.repairmanName}】的派单吗？`, '提示', { type: 'warning' }).then(() => {
    axios.post('http://localhost:8080/api/repair/revoke', { id: row.id }).then(res => {
       ElMessage.success('已撤回指派，单据重置为待处理')
       fetchData()
    })
  })
}
