...
const pageSize = ref(8) // 卡片每页显示 8 个比较合适
...
const fetchData = () => {
  loading.value = true
  axios.get('http://localhost:8080/api/common/message/list', {
    params: {
...
.box-card { min-height: calc(100vh - 100px); border-radius: 8px; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); background-color: #fdfdfd; }
...
/* 只保留旋转效果，背景色由 inline style 控制 */
.note-card:nth-child(even) { transform: rotate(2deg); }

.note-card:hover { transform: scale(1.05) rotate(0deg); z-index: 10; box-shadow: 4px 4px 15px rgba(0,0,0,0.2); }
...
.note-title { margin-top: 15px; font-size: 18px; color: #333; border-bottom: 2px dashed rgba(0,0,0,0.1); padding-bottom: 5px; font-family: 'Comic Sans MS', cursive, sans-serif; }
.note-content { margin-top: 10px; color: #555; line-height: 1.6; word-break: break-all; min-height: 80px; font-size: 14px; }
.note-footer { margin-top: 20px; text-align: right; font-weight: bold; font-size: 12px; color: rgba(0,0,0,0.5); }
</style>