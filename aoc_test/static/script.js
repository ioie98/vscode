...
function clearStationList() {
    ...
    // 重置时间范围输入框
    const startInput = document.getElementById('unifiedStart');
    const endInput = document.getElementById('unifiedEnd');
    if (startInput) startInput.value = '';
    if (endInput) endInput.value = '';

    //重置状态栏
    ['pwv', 'weather', 'rain'].forEach(t => {
        updateStatusUI(t, "未操作", "0%");
        
        const rangeId = {
            pwv: "pwvTimeRange",
            weather: "weatherTimeRange",
            rain: "rainTimeRange"
        }[t];
        ...
        tr.innerHTML = `
            ...
            <td>
                <div style="display: flex; gap: 8px; justify-content: flex-start;">
                    ...
                    <button class="btn upload-btn small" onclick="uploadDatasetToOSS('${ds.site}','${ds.sub_dir}','${ds.file}', this)">上传</button><span class="upload-status"></span>
                </div>
            </td>
        `;
...
async function uploadDatasetToOSS(siteId, mode, filename, btnEl) {
    // 找到按钮后面的状态显示 span
    const statusEl = btnEl?.nextElementSibling;
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            if (cbRes.error) {
                showToast(`TOS上传成功,但回调失败: ${cbRes.error}`, 'warning');
            } else {
