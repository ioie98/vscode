...
const fetchData = () => {
  axios.get('http://localhost:8080/api/user/list', {
    ...
    if (res.data.code === 200) {
      ...
      total.value = res.data.data.total // 注意：这里的total是总数，但因为我们在前端做了filterRole，分页显示可能不准。完美方案是后端支持role筛选。
      
      if (filterRole.value === '') {
        tableData.value = allData
      } else {
        tableData.value = allData.filter(item => item.role === filterRole.value)
      }
    }
  })
}
