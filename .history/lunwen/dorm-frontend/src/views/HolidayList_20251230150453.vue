<template>
  <div class="app-container">
    <el-card class="box-card">
      <div class="clearfix" style="display: flex; justify-content: flex-start; align-items: center;">
        <!-- 左侧操作区：登记按钮，标签，刷新按钮，按顺序排列 -->
        <div class="header-actions" style="display: flex; align-items: center; gap: 10px;">
          ...
          <el-tag v-if="user.role === 0" size="large" type="primary">全校去向数据</el-tag>
      ...
      <!--去向筛选区域 -->
      <div class="filter-header" style="padding-bottom: 15px; border-bottom: 1px dashed #eee; display: flex; justify-content: flex-end; align-items: center;">
        <span>去向筛选：</span>
...
>
  ...
  <el-table-column prop="destination" label="去向" :align="'center'">
    <template #default="{ row }">
      <el-tag
        :type="row.destination === '留校' ? 'primary' : 'warning'"
        size="small"
      >
        {{ row.destination }}
      </el-tag>
...
/* 修改：filter-header 变为右对齐 */
.filter-header { 
  display: flex; 
  align-items: center; 
  justify-content: flex-end; /* 关键：让内容右对齐 */
} 
</style>