    ...
    <el-table :data="tableData" border stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="位置" width="150">
        <template #default="scope">
           <!-- 显示楼栋和房号，如果没有则显示默认 -->
           {{ scope.row.buildingName || '未知' }} {{ scope.row.roomNumber || '' }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="报修时间" width="170">
...
const fetchData = () => {
  loading.value = true
  // 构造参数：如果是学生，传 userId；如果是管理员/维修工，不传 userId (查所有)
  const params = userRole === 1 ? { userId: currentUser.id } : {}
...
const createData = () => {
  ...
  axios.post('http://localhost:8080/api/repair/add', {
    ...
    dormId: currentUser.dormId // 关键：把当前用户的宿舍ID传过去
  }).then(res => {
    if (res.data.code === 200) {
...
const handleTakeOrder = (row) => {
  ElMessageBox.confirm('确定接单吗？', '提示').then(() => {
    axios.post('http://localhost:8080/api/repair/take', { 
        id: row.id, 
...
const isMyTask = (row) => {
  return row.repairmanId === currentUser.id
}
