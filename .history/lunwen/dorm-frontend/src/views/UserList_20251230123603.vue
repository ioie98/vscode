          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
               <el-form-item label="床位号">
                ...
                <el-input v-model="form.bedNum" placeholder="1-4" type="number" />
              </el-form-item>
            </el-col>
          </el-row>
...
// 修复：确保 form 的初始值包含所有字段，且 bedNum 默认为空
const form = reactive({
  username: '', realName: '', role: 1, phone: '', 
  college: '', major: '', className: '',
  zoneName: '', buildingName: '', roomNumber: '', bedNum: '' // 初始化这些字段
})
...
const submitAdd = () => {
  ...
  // 仅当选择学生角色且宿舍信息不全时，进行提示
  if (form.role === 1 && (!form.zoneName || !form.buildingName || !form.roomNumber)) {
    ElMessage.warning('请填写完整的宿舍信息，否则将无法分配宿舍')
    // 这里可以选择return，或者允许不分配宿舍继续提交
    // return; 
  }
