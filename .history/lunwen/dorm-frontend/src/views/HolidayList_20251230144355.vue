<template>
  <div class="app-container">
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="display: flex; justify-content: flex-end; align-items: center;">
        <!-- 顶部Header只保留右侧刷新按钮 -->
        <el-button icon="Refresh" circle @click="fetchData" />
      </div>

      <!-- 修改点：将标签和按钮移到筛选条件上方，并左对齐 -->
      <div style="margin-bottom: 20px;">
        <div class="header-status-tags" style="margin-bottom: 15px;">
          <el-tag v-if="user.role === 1" size="large" type="success" style="margin-right: 10px;">本宿舍去向</el-tag>
          <el-tag v-if="user.role === 0" size="large" type="danger" style="margin-right: 10px;">全校去向数据</el-tag>
        </div>
        <div class="header-actions">
          <!-- 只有学生(Role 1)才能看到“登记去向”按钮 -->
