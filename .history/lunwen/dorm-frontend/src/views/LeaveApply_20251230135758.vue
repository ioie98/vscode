    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div class="header-actions">
          <el-button v-if="user.role === 1 || user.role === 2" type="primary" icon="Plus" @click="dialogVisible = true">
            提交申请
          </el-button>
          <el-tag v-if="user.role === 0" size="large" style="margin-right: 10px;">全校审批</el-tag>
          <el-tag v-else-if="user.role === 1" size="large" type="success" style="margin-right: 10px;">本宿舍申请</el-tag>
          <el-tag v-else-if="user.role === 2" size="large" type="warning" style="margin-right: 10px;">维修工申请</el-tag>
          <el-button icon="Refresh" circle @click="fetchList" style="margin-left: 10px;" />
        </div>
      </div>
      ...
      <div v-if="list.length === 0" style="text-align: center; color: #999; margin-top: 20px;">
        暂无申请记录
      </div>
      ...
      <el-dialog v-model="dialogVisible" title="提交申请" width="500px">
        <el-form :model="form" label-width="80px">
          ...
          <el-form-item label="起止时间" prop="leaveTime">
            <el-date-picker v-model="form.leaveTime" type="date" placeholder="开始日期" style="width: 130px"/>
            - 
            <el-date-picker v-model="form.returnTime" type="date" placeholder="结束日期" style="width: 130px"/>
          </el-form-item>
        </el-form>
        <template #footer>
...
const handleAudit = (row, status) => {
  axios.post('http://localhost:8080/api/common/leave/audit', {
    ...
    ElMessage.error("审核失败，请检查网络或控制台错误");
  });
}
