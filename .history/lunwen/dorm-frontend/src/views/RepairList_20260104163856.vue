    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div class="header-actions">
          ...
          <el-checkbox v-if="userRole !== 1" v-model="showOnlyPending" label="只看待处理" @change="fetchData" style="margin-left: 0px;" align-items: center; border />
          <el-button icon="Refresh" circle @click="fetchData" style="margin-left: 10px;"></el-button>
        </div>
      </div>
...
const fetchData = () => {
  ...
  axios.get('http://192.168.12.26:8080/api/repair/list', { params })
    .then(res => {
...
const createData = () => {
  ...
  axios.post('http://192.168.12.26:8080/api/repair/add', {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除/撤销吗？', '提示', { type: 'warning' }).then(() => {
    axios.post('http://192.168.12.26:8080/api/repair/delete', { id: row.id }).then(res => {
...
const openAssignDialog = (row) => {
  ...
  axios.get('http://192.168.12.26:8080/api/user/repairmen').then(res => {
    if(res.data.code === 200) {
...
const submitAssign = () => {
  ...
  axios.post('http://192.168.12.26:8080/api/repair/assign', {
...
const handleTakeOrder = (row) => {
  ElMessageBox.confirm('确定接单吗？', '提示').then(() => {
    axios.post('http://192.168.12.26:8080/api/repair/take', { id: row.id, repairmanId: currentUser.id }).then(res => {
...
const handleFinish = (row) => {
  ElMessageBox.confirm('确认已维修完成？', '提示').then(() => {
    axios.post('http://192.168.12.26:8080/api/repair/finish', { id: row.id }).then(res => {
