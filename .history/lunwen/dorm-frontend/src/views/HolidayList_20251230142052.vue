    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div class="header-actions">
          ...
          <!-- 新增：只有学生(Role 1)才能看到“登记去向”按钮 -->
          <el-button v-if="user.role === 1" type="primary" icon="Edit" @click="openAddDialog" style="margin-left: 10px;">登记去向</el-button>
...
const addForm = reactive({
  ...
  major: user.major || '', // 确保有默认值
  className: user.className || '', // 确保有默认值
  phone: user.phone || '', // 确保有默认值
  destination: '留校', // 默认留校
  location: '',
  returnTime: ''
})
...
const fetchData = () => {
  axios.get(`http://localhost:8080/api/common/holiday/list`, {
    params: {
      ...
      role: user.role,     
      dormId: user.dormId,  
      // 这里的 studentId 和 name 暂时不需要传，后端Mapper没有用到，但是如果你需要更精确的过滤
      // 例如：学生只能看自己的去向，可以在这里加上 userId: user.id
    }
  }).then(res => {
...
const submitAddForm = () => {
  ...
  axios.post('http://localhost:8080/api/common/holiday/add', addForm).then(res => {
