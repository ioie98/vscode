<template>
  <div class="app-container">
    <el-card class="box-card">
      ...
      >
        <el-table-column label="宿舍" width="280">
        ...
        <el-table-column label="用量详情" width="240" align="center">
          <template #default="{ row }">
        ...
        <el-table-column label="费用详情" width="240" align="center">
          <template #default="{ row }">
        ...
        <el-table-column prop="totalCost" label="总金额" width="160" align="center">
          <template #default="{ row }">
...
const fetchData = () => {
  ...
  loading.value = true
  axios.get('http://192.168.12.26:8080/api/utility/list', {
    params: {
      role: user.role,
      dormId: user.dormId,
...
const handlePay = (row) => {
  ElMessageBox.confirm(`确认支付 ￥${row.totalCost} 吗？`, '缴费确认', { confirmButtonText: '支付' })
    .then(() => {
      axios.post('http://192.168.12.26:8080/api/utility/pay', { id: row.id }).then(res => {
        if(res.data.code === 200) {
...
const openDialog = (isEdit, row) => {
  if (dormList.value.length === 0) {
    axios.get('http://192.168.12.26:8080/api/dorm/list').then(res => {
...
const submitForm = () => {
  ...
  const url = isEditMode.value ? 'http://192.168.12.26:8080/api/utility/update' : 'http://192.168.12.26:8080/api/utility/add'
