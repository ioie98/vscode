  ...
  <el-card>
    <div class="filter-header">
      <div style="margin-right: auto; font-weight: bold;">
        节假日去向统计
        <el-tag v-if="user.role === 1" type="success" style="margin-left: 10px">本宿舍</el-tag>
        <el-tag v-if="user.role === 0" type="danger" style="margin-left: 10px">全校数据</el-tag>
      </div>
...
const fetchData = () => {
  axios.get(`http://localhost:8080/api/common/holiday/list`, {
    params: {
      destination: query.value,
      role: user.role,     // 传角色
      dormId: user.dormId  // 传宿舍ID
    }
  }).then(res => {
