...
def upload_data():
    try:
        ...
        df['data_time'] = df['data_time'] - pd.Timedelta(hours=8)  # 北京时间 -> UTC
        ...
        def map_fields(df, table_type):
            ...
            for col in expected:
                if col not in df.columns:
                    # 缺失列补默认值
                    if col in ['pwv','ta','pa','ua','rainfall']:
                        df[col] = 0.0
                    elif col in ['rain_time','reset']:
                        df[col] = 0
                    elif col in ['lon','lau']:
                        df[col] = "0"
                    else:
        ...
        if not insert_df.empty:
            start_time = (insert_df['data_time'].apply(lambda x: pd.to_datetime(x)) + pd.Timedelta(hours=8)).min().strftime('%Y/%m/%d %H:%M:%S')
            end_time = (insert_df['data_time'].apply(lambda x: pd.to_datetime(x)) + pd.Timedelta(hours=8)).max().strftime('%Y/%m/%d %H:%M:%S')
        else:
