...
# 数据上传接口（修复字段不一致、lon/lau、rain_time/reset 无法上传的问题）
...
def upload_data():
    try:
        ...
        table = cfg['table']
        fields = [f.lower() for f in cfg['fields']]  # 统一小写
        df = pd.read_csv(file, encoding='utf-8-sig')

        # --------------------------- 1. 列名规范化 ---------------------------
        orig_cols = list(df.columns)
        col_map = {c: c.strip().lower() for c in orig_cols}
        df.rename(columns=col_map, inplace=True)

        # 别名映射（将 CSV 中多种写法，映射为表字段）
        alias_to_field = {
            # 经纬度
            'lon': 'lon', 'longitude': 'lon', 'lng': 'lon',
            'lat': 'lau', 'latitude': 'lau',

            # PWV
            'pwv': 'pwv',

            # HWS
            'ta': 'ta', 'temp': 'ta', 'temperature': 'ta',
            'pa': 'pa', 'pressure': 'pa',
            'ua': 'ua', 'humidity': 'ua', 'rh': 'ua',

            # rain
            'rainfall': 'rainfall', 'tp_15min': 'rainfall',
            'rain_time': 'rain_time', 'reset': 'reset',
        ...
        col_to_canon = {}
        for c in df.columns:
            col_to_canon[c] = alias_to_field.get(c, c)  # 若无别名映射则保留原名

        df.rename(columns=col_to_canon, inplace=True)

        # --------------------------- 2. 识别时间列 ---------------------------
        time_col = None
        for c in df.columns:
            if c in ['data_time', 'date', 'time'] or '时间' in c:
                time_col = c
                break
        ...
        for _, row in df.iterrows():
            ...
            if dtype == 'pwv':
                buffer_list.append({
                    ...
                    'lon': (str(row.get('lon')).strip() if pd.notna(row.get('lon')) else None),
                    'lau': (str(row.get('lau')).strip() if pd.notna(row.get('lau')) else None),
                    'pwv': to_float_or_none(row.get('pwv')),
            ...
            # ---------------- HWS ----------------
                ...
                buffer_list.append({
                    'data_time': dt_utc,
                    'device': str(row.get('device', device)),
                    'ta': to_float_or_none(row.get('ta')),
                    'pa': to_float_or_none(row.get('pa')),
                    'ua': to_float_or_none(row.get('ua')),
                    'data_type': row.get('data_type', 'uploaded')
                })
            ...
            # ---------------- Rain ----------------
                ...
                buffer_list.append({
                    'data_time': dt_utc,
                    'device': str(row.get('device', device)),
                    'rainfall': to_float_or_none(row.get('rainfall')),
                    'data_type': row.get('data_type', 'uploaded'),
                    'rain_time': to_int_or_none(row.get('rain_time')),
                    'reset': to_int_or_none(row.get('reset'))
                })
        ...
        try:
            q = f"""
                ...
                WHERE device = '{device}'
                  AND data_time >= toDateTime('{min_t.strftime("%Y-%m-%d %H:%M:%S")}', 'UTC')
                  AND data_time <= toDateTime('{max_t.strftime("%Y-%m-%d %H:%M:%S")}', 'UTC')
            """
            res = client.query(q)
        ...
        for r in buffer_list:
            row_list = []
            for f in fields:
                row_list.append(r.get(f, None))  # None → NULL
            records_list.append(row_list)
        ...
        # --------------------------- 6. 返回结果 ---------------------------
        start_time = min(r['data_time'] for r in buffer_list).strftime('%Y/%m/%d %H:%M:%S') if buffer_list else None
        end_time = max(r['data_time'] for r in buffer_list).strftime('%Y/%m/%d %H:%M:%S') if buffer_list else None
