...
def upload_data():
    try:
        ...
        insert_df['data_time'] = insert_df['data_time'] - pd.Timedelta(hours=8)  # UTC
        ...
        if not to_insert_df.empty:
            ...
            # ---------------------------------------------------------
            # ⭐⭐⭐ 通用补丁：ClickHouse Float64 不能接收 None / "" / "null" ⭐⭐⭐
            # 自动把所有 Float64 字段的空值转换成 0.0
            # ---------------------------------------------------------
            float_fields = []
            for f in cfg['fields']:
                if f.lower() in ['pwv', 'rainfall', 'ta', 'pa', 'ua']:
