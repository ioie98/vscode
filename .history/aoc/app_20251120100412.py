...
# 数据上传接口（直接上传到 ClickHouse，CSV 时间为北京时间 -> 存为 UTC）
...
def upload_data():
    try:
        ...
        dtype = request.form.get('type')  # rain/pwv/hws/weather
        ...
        # 时间处理：北京时间 -> UTC（只减一次！）
        insert_df = df.copy()
        insert_df['data_time'] = pd.to_datetime(insert_df[time_col], errors='coerce')
        insert_df = insert_df.dropna(subset=['data_time'])
        insert_df['data_time'] = insert_df['data_time'] - pd.Timedelta(hours=8)  # UTC ONLY ONCE

        # 记录开始/结束时间（北京时间）
        if not insert_df.empty:
            start_time = (insert_df['data_time'] + pd.Timedelta(hours=8)).min().strftime('%Y/%m/%d %H:%M:%S')
            end_time = (insert_df['data_time'] + pd.Timedelta(hours=8)).max().strftime('%Y/%m/%d %H:%M:%S')

        # device 补充
        if 'device' not in insert_df.columns:
            insert_df['device'] = device

        # data_type 补充
        if 'data_type' not in insert_df.columns:
            insert_df['data_type'] = 'uploaded'

        # -------- 修复点 ①：Decimal 字段必须用 str ----------
        if 'lon' in insert_df.columns:
            insert_df['lon'] = insert_df['lon'].astype(str)
        if 'lau' in insert_df.columns:
            insert_df['lau'] = insert_df['lau'].astype(str)
        ...
        if not to_insert_df.empty:
            ...
            # 构建 records_list（二位列表）
            records_list = []
            for _, row in to_insert_df.iterrows():
                rec = [None if pd.isna(row.get(f)) else row[f] for f in cfg['fields']]
                records_list.append(rec)

            # 修复点 ②：float 字段转换（不处理 lon/lau）
            float_fields = ['pwv', 'rainfall', 'ta', 'pa', 'ua']
            for row in records_list:
                for f in float_fields:
                    if f in cfg['fields']:
                        idx = cfg['fields'].index(f)
                        val = row[idx]
                        if val is None or val == '' or str(val).lower() == 'null' or pd.isna(val):
                            row[idx] = 0.0
                        else:
                            try:
                                row[idx] = float(val)
            ...
            try:
                client.insert(cfg['table'], records_list, column_names=cfg['fields'])
