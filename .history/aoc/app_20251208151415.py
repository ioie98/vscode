...
tos_client = tos.TOSClient(
    ...
    access_key_id="AKLTNzQ3YTk1NjQ4YmU4NDBmMDk0NTI2ZjlhN2YzODdkY2E",
    secret_access_key="T1RCbE9EYzFOalUzTXpFME5EaGxaR0ZsWkRFME5EVmtZVEkxTnpSaU5HTQ==",
    region="cn-beijing"
)
...
def upload_to_tos():
    try:
        ...
        mode = request.form.get('mode')  # train / pred
        site = request.form.get('site')  # 任意站点ID

        if not file or not mode or not site:
            return jsonify({'error': '缺少文件/模式/站点'}), 400

        key = f"{mode}/{site}/quantitative/{file.filename}"

        # 上传到 TOS
        tos_client.put_object(bucket_name=bucket_name, key=key, body=file)

        return jsonify({'status': 'success', 'key': key})
    except Exception as e:
        traceback_str = traceback.format_exc()
        return jsonify({'status': 'fail', 'error': str(e), 'trace': traceback_str}), 500
