...
def upload_data():
    try:
        ...
        # --------------------- 1. 时间处理 ---------------------
        df['data_time'] = pd.to_datetime(df[time_col], errors='coerce')
        df = df.dropna(subset=['data_time'])
        df['data_time'] = df['data_time'] - pd.Timedelta(hours=8)  # UTC
        ...
        def map_fields(df, table_type):
            ...
            if table_type == 'pwv':
                ...
                df['lon'] = df['lon'].apply(lambda x: str(x) if not pd.isna(x) else "0")
                df['lau'] = df['lau'].apply(lambda x: str(x) if not pd.isna(x) else "0")
