...
# 数据上传接口（CSV 时间为北京时间 -> 存为 UTC，并自动映射表字段）
...
def upload_data():
    try:
        ...
        for f in required_fields:
            if f not in insert_df.columns:
                insert_df[f] = np.nan

        # 遍历 CSV 列进行映射
        for col in df.columns:
            lc = col.strip().lower()
            for target_field, candidates in field_maps.get(dtype, {}).items():
                if lc in [c.lower() for c in candidates]:
                    if target_field in ['rain_time','reset']:
                        insert_df[target_field] = df[col]
                    else:
                        insert_df[target_field] = pd.to_numeric(df[col], errors='coerce') if target_field not in ['data_time','device','data_type','lon','lat'] else df[col]
        ...
        # 确保 datetime 类型
        if not isinstance(min_t, pd.Timestamp):
            min_t = pd.to_datetime(min_t)
        if not isinstance(max_t, pd.Timestamp):
