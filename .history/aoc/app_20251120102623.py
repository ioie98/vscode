...
def upload_data():
    try:
        ...
        dtype = request.form.get('type')  # rain/pwv/hws
        ...
        if dtype not in TABLE_CONFIG:
            return jsonify({'error': f'未知数据类型: {dtype}'}), 400
        ...
        for col in df.columns:
            if col.lower() in ['data_time', 'date', 'time'] or '时间' in col:
                ...
                break
        if time_col is None:
            return jsonify({'status':'success','inserted_count':0,'error':'上传文件无时间列'})

        # --------------------- 1. 解析时间并转换为 UTC ---------------------
        def parse_beijing_to_utc(ts):
            dt = pd.to_datetime(ts, errors='coerce')
            if pd.isna(dt):
                return pd.NaT
            return dt - pd.Timedelta(hours=8)  # 北京时间 -> UTC

        df['data_time'] = df[time_col].apply(parse_beijing_to_utc)
        df = df.dropna(subset=['data_time'])

        # --------------------- 2. 补充 device 和 data_type ---------------------
        if 'device' not in df.columns:
            df['device'] = device
        if 'data_type' not in df.columns:
            df['data_type'] = 'uploaded'

        # --------------------- 3. 严格映射字段 ---------------------
        def map_fields(df, table_type):
            df = df.copy()
            if table_type == 'pwv':
                expected = ['data_time', 'device', 'lon', 'lau', 'pwv', 'data_type']
                for col in ['lon', 'lau', 'pwv']:
                    if col not in df.columns:
                        df[col] = 0
                df = df.reindex(columns=expected)
                df['lon'] = df['lon'].apply(lambda x: str(x) if pd.notna(x) else "0")
                df['lau'] = df['lau'].apply(lambda x: str(x) if pd.notna(x) else "0")
                df['pwv'] = df['pwv'].apply(lambda x: float(x) if pd.notna(x) else 0.0)

            elif table_type == 'hws':
                expected = ['data_time', 'device', 'ta', 'pa', 'ua', 'data_type']
                for col in ['ta','pa','ua']:
                    if col not in df.columns:
                        df[col] = 0.0
                df = df.reindex(columns=expected)
            ...
            elif table_type == 'rain':
                expected = ['data_time','device','rainfall','data_type','rain_time','reset']
                for col in ['rainfall','rain_time','reset']:
                    if col not in df.columns:
                        df[col] = 0
                df = df.reindex(columns=expected)
                df['rainfall'] = df['rainfall'].apply(lambda x: float(x) if pd.notna(x) else 0.0)
                df['rain_time'] = df['rain_time'].apply(lambda x: int(x) if pd.notna(x) else 0)
                df['reset'] = df['reset'].apply(lambda x: int(x) if pd.notna(x) else 0)

            return df

        insert_df = map_fields(df, dtype)
        ...
        try:
            q = f"""
                ...
                WHERE device = '{device}'
                  AND data_time >= toDateTime('{(min_t - pd.Timedelta(seconds=1)).strftime("%Y-%m-%d %H:%M:%S")}', 'UTC')
                  AND data_time <= toDateTime('{(max_t + pd.Timedelta(seconds=1)).strftime("%Y-%m-%d %H:%M:%S")}', 'UTC')
            """
            res = client.query(q)
            existing_times = set(
                row[0].strftime("%Y-%m-%d %H:%M:%S") if isinstance(row[0], datetime) else str(row[0])
                for row in res.result_rows
            )
        except:
            existing_times = set()

        insert_df = insert_df[~insert_df['data_time'].apply(lambda x: x.strftime("%Y-%m-%d %H:%M:%S") in existing_times)]

        # --------------------- 5. 构建二维数组并插入 ---------------------
        insert_df['data_time'] = insert_df['data_time'].dt.strftime("%Y-%m-%d %H:%M:%S")
        records_list = insert_df[fields].values.tolist()

        inserted_count = 0
        if records_list:
            try:
                client.insert(table, records_list, column_names=fields)
                inserted_count = len(records_list)
            except Exception as e:
        ...
        # --------------------- 6. 返回上传结果 ---------------------
        start_time = (insert_df['data_time'].apply(lambda x: pd.to_datetime(x)) + pd.Timedelta(hours=8)).min().strftime('%Y/%m/%d %H:%M:%S') if not insert_df.empty else None
        end_time = (insert_df['data_time'].apply(lambda x: pd.to_datetime(x)) + pd.Timedelta(hours=8)).max().strftime('%Y/%m/%d %H:%M:%S') if not insert_df.empty else None

        return jsonify({'status':'success','start_time':start_time,'end_time':end_time,'inserted_count':inserted_count})
