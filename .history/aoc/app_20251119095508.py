...
def get_stations():
    try:
        ...
        cursor.execute("SELECT ID AS site_id, NAME AS site_name FROM sb_jbxx")
...
# 接收站点列表并合并到 site/<mode> 目录（只合并本次下载的文件）
...
def get_datasets_route():
    ...
    if site:
        try:
            conn = mysql.connector.connect(**MYSQL_CONFIG)
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT NAME FROM sb_jbxx WHERE ID=%s", (site,))
            row = cursor.fetchone()
            if row:
                site_name = row['NAME']
            cursor.close()
    ...
    site_dir = os.path.join(TMP_DIR, site) if site else None
    if not site or not os.path.exists(site_dir):
            ...
            for file in os.listdir(mode_dir):
                if file.endswith('.csv') and (re.match(r'^dataset[-_]', file, re.I) or re.match(r'^[A-Za-z0-9]+_\d{8}_\d{8}\.csv$', file)):
                    ...
                    try:
                        ...
                        start_time = str(pd.to_datetime(df[time_col], errors='coerce').min()) if time_col else None
                        end_time = str(pd.to_datetime(df[time_col], errors='coerce').max()) if time_col else None
                    except:
                        start_time = None
                        end_time = None
                    out.append({
                        ...
                        "name": file,
                        "site": site,
                        "site_name": site_name,  # <-- 使用 MySQL 名称
                        "start_time": start_time,
                        "end_time": end_time,
