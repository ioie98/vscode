...
def merge_datasets_inner(sites, mode='train'):
    ...
    for site in sites:
        ...
        for fpath in csv_files:
            ...
            std_cols = {c: col_map[c.strip().lower()] for c in df.columns if c.strip().lower() in col_map}
        ...
        # 新命名规则：站点_开始日期_结束日期.csv
        try:
            first_date = pd.to_datetime(merged['date'], errors='coerce').min()
            last_date = pd.to_datetime(merged['date'], errors='coerce').max()
            start_str = first_date.strftime('%Y%m%d') if pd.notna(first_date) else 'start'
            end_str = last_date.strftime('%Y%m%d') if pd.notna(last_date) else 'end'
        except:
            start_str = 'start'
            end_str = 'end'

        dataset_name = f"{site}_{start_str}_{end_str}.csv"
        ...
        results.append({'site_id': site,'sub_dir': mode,'dataset': dataset_name,'time_range': f"{first_date} ~ {last_date}"})
