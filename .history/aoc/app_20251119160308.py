...
def upload_data():
    try:
        ...
        # 插入 ClickHouse（改用二维列表避免 KeyError）
        if not to_insert_df.empty:
            ...
            for _, row in to_insert_df.iterrows():
                ...
                for f in cfg['fields']:
                    if f in row:
                        rec.append(None if pd.isna(row[f]) else row[f])
                    else:
                        if f == 'data_time':
                            rec.append(row.get('data_time'))
                        elif f == 'device':
                            rec.append(row.get('device'))
                        elif f == 'data_type':
                            rec.append(row.get('data_type', 'uploaded'))
                        else:
                            rec.append(None)
