...
def _download_single_type_for_device(device, data_type, start_time, end_time, priority_list, mode='train'):
    ...
    # 文件名只保留站点_起始_结束.csv，不包含 data_type
    file_name = f"{device}_{start_local.strftime('%Y%m%d')}_{end_local.strftime('%Y%m%d')}.csv"
    tmp_file = os.path.join(site_dir, file_name)
...
def merge_datasets_inner(devices, mode='train'):
    results = []
    for device in devices:
        site_dir = os.path.join(TMP_DIR, device, mode)
        if not os.path.exists(site_dir):
            results.append({'site_id': device, 'error': f'{mode} 目录不存在'})
            continue

        # 只查找 CSV 文件
        csv_files = [f for f in os.listdir(site_dir) if f.lower().endswith('.csv')]

        if not csv_files:
            results.append({'site_id': device, 'error': f'{mode} 目录下无 CSV 文件'})
            continue
        ...
        for f in csv_files:
            ...
            if 'data_time' in df.columns:
                first_date = pd.to_datetime(df['data_time'].iloc[0], errors='coerce')
                last_date = pd.to_datetime(df['data_time'].iloc[-1], errors='coerce')
    ...
    return {'status': 'success', 'results': results} if results else {'status': 'success', 'message': '无 CSV 文件'}
