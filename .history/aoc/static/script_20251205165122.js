    ...
    fetch(`/delete_dataset/${filename}/${siteVal}`, { method: 'DELETE' })
        ...
        .then(res => {
            ...
            if (res.status === 'success' || res.message?.includes('不存在')) {
                showToast(res.status === 'success' ? '删除成功' : '文件已不存在，已移除');

                // 移除 datasets 中对应记录
                const fileBaseName = filename.split('/').pop();
                datasets = datasets.filter(d => {
                    const path = [d.sub_dir || '', d.file].filter(Boolean).join('/');
                    return !(path.endsWith(fileBaseName) && d.site === siteVal);
                });
...
function renderDatasetTable(page){
    ...
    currentPage = Math.min(page, totalPages); // 防止当前页超出总页数
...
function renderPagination(){
    ...
    if(datasets.length === 0) return; // datasets为空时分页为空
