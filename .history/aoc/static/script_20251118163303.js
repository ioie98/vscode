...
if(res.merged && ((res.merged.results && Array.isArray(res.merged.results)) || Array.isArray(res.merged))) {
    ...
    mergedList.forEach(m => {
        // 按每次下载的请求起止时间生成文件名，确保每次下载单独文件
        const siteId = m.site_id || 'site';
        const startStr = start.replace(/[-:\s]/g,'').slice(0,8);  // 起始时间 YYYYMMDD
        const endStr = end.replace(/[-:\s]/g,'').slice(0,8);      // 结束时间 YYYYMMDD
        const newFileName = `${siteId}_${startStr}_${endStr}.csv`;

        const now = new Date();
        const rec = {
            file: newFileName,
            site: siteId,
            site_name: m.site_name || siteId,
            sub_dir: m.sub_dir || '', 
            start_time: startStr,
            end_time: endStr,
