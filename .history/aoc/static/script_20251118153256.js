    ...
    .then(res=> {
        ...
        if(res.merged && ((res.merged.results && Array.isArray(res.merged.results)) || Array.isArray(res.merged))) {
            ...
            mergedList.forEach(m => {
                const now = new Date();
                const timestamp = now.getTime(); // 毫秒时间戳，保证唯一
                const newFileName = `${m.site_id || 'site'}_${timestamp}_${m.type || 'data'}.csv`;

                const rec = {
                    file: newFileName,
                    site: m.site_id,
                    site_name: m.site_name || m.site_id,
                    sub_dir: m.sub_dir || mode,   // train/pred
                    start_time: (m.time_range || '').split('~')[0]?.trim() || start,
                    end_time: (m.time_range || '').split('~')[1]?.trim() || end,
                    created_at: now.toLocaleString(),
                    path: m.dataset // 后端实际存储路径
                };

                datasets.push(rec); // 直接添加，不去重
            });

            // 按创建时间倒序
            datasets.sort((a,b) => new Date(b.created_at) - new Date(a.created_at));
            renderDatasetTable(1); // 刷新第一页显示
            showToast('下载完成并记录文件独立保存');
        }
