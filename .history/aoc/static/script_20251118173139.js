    ...
    .then(res=> {
        ...
        if(res.merged && ((res.merged.results && Array.isArray(res.merged.results)) || Array.isArray(res.merged))) {
            ...
            mergedList.forEach(m => {
                // 文件名
                const newFileName = m.dataset || `${m.site_id}_${m.time_range.replace(/[-:\s]/g,'').slice(0,8)}_${m.time_range.split('~')[1]?.trim().replace(/[-:\s]/g,'').slice(0,8)}.csv`;
                const now = new Date();
                const rec = {
                    file: newFileName,
                    site: m.site_id,
                    site_name: m.site_name || m.site_id,  // 中文站点名称
                    sub_dir: m.sub_dir || '',
                    start_time: m.time_range ? m.time_range.split('~')[0]?.trim().replace(/[-:\s]/g,'').slice(0,8) : '',
                    end_time: m.time_range ? m.time_range.split('~')[1]?.trim().replace(/[-:\s]/g,'').slice(0,8) : '',
                    created_at: now.toLocaleString(),
                    path: m.dataset
                };

                // 避免重复添加 path+site+sub_dir 相同的记录
