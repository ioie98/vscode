...
function startUnifiedDownload(){
    ...
    fetch('/batch_download', {
        ...
        body: JSON.stringify({ types, start_time: start, end_time: end, devices, priority: priorityOrder })
    })
    .then(res=>res.json())
    .then(res=> {
        if(res.error){
        ...
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            ...
            mergedList.forEach(m => {
                ...
                const rec = {
                    file: `dataset-${timestamp}.csv`,
                    site: m.site_id,
                    site_name: m.site_name || m.site_id,  // ← 确保站点名称
                    sub_dir: m.sub_dir || '',
...
function renderDatasetTableMultiSite(fileList){
    ...
    fileList.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td>${createdAt}</td>
            <td>
                <button class="btn detail-btn" onclick="showDetails('${encodeURIComponent(filePath)}','${siteId}')">详情</button>
...
function loadSiteDatasets() {
    ...
    fetch(`/get_datasets?site=${site}`)
        ...
        .then(res => {
            ...
            const dsList = (res.datasets || []).filter(d => {
                const fname = d.name.split('/').pop();
                return fname && /^dataset[-_]/i.test(fname); // 匹配 dataset- 或 dataset_ 开头
            });
            ...
            // 自动加载所有历史合并文件
            datasets.forEach(ds => {
                const path = [ds.sub_dir || '', ds.name].filter(Boolean).join('/');
                loadDatasetForAnalysis(path, true);
            });
        })
...
function deleteDataset(filename){
    ...
    fetch(`/delete_dataset/${encodeURIComponent(filename)}/${site}`, { method:'DELETE' })
        .then(res=>res.json())
        .then(res=>{
...
function renderDatasetTable(page){
    ...
    pageData.forEach(ds => {
        ...
        const filePath = [ds.sub_dir || '', ds.name].filter(Boolean).join('/');
...
//详情弹框
function showDetails(fileEncoded, site){
    const file = decodeURIComponent(fileEncoded);
    fetch(`/tmp_data_preview/${encodeURIComponent(file)}/${site}`)
    .then(res=>res.json())
    .then(data=>{
        const modalBg = document.getElementById('modalBg');
        const modalBody = document.getElementById('modalBody');
