...
// 删除文件
// 删除文件
function deleteDataset(filenameWithDir, site){
    const siteVal = site || (document.getElementById('stationSelect')?.value || '').trim();
    if(!siteVal){ showToast('站点未选择','error'); return; }

    // 分离 sub_dir 和 file
    const parts = filenameWithDir.split('/');
    const subDir = parts.length > 2 ? parts[0] : 'train';  // 默认 train
    const file = parts[parts.length-1];

    fetch(`/delete_dataset/${encodeURIComponent(file)}/${siteVal}?sub_dir=${encodeURIComponent(subDir)}`, { method:'DELETE' })
        .then(res=>res.json())
        .then(res=>{
            if(res.status==='success'){
                ...
                // 移除对应记录
                datasets = datasets.filter(d => !(d.file === file && d.site === siteVal && (d.sub_dir || 'train') === subDir));
