...
function deleteDataset(filename, site) {
    ...
    fetch(`/delete_dataset/${encodeURIComponent(filename)}/${siteVal}`, { method:'DELETE' })
        .then(res => res.json())
        .then(res => {
...
function renderDatasetTable(page){
    ...
    pageData.forEach(ds => {
        ...
        tr.innerHTML = `
            ...
            <td>${createdAt}</td>
            <td>
                <button class="btn delete-btn" onclick='deleteDataset(${JSON.stringify(filePath)}, ${JSON.stringify(siteId)})'>删除</button>
...
function downloadDataset(siteId, subDir, file){
    ...
    window.location.href = "/download_dataset/" + encodeURIComponent(filePath);
}
