...
function onStationInputKeydown(e){ if(e.key === 'Enter'){ e.preventDefault(); addStationFromInput(); } }
...
function removeStation(s){
    ...
    if(input) input.value = stations[0] || '';
    loadSiteDatasets();
}
...
function startUpload(type){
    ...
    const stationId = (document.getElementById('stationSelect')?.value || '').trim();
    ...
    .then(res=>{
        if(res.error){
            ...
            if(rangeId && res.start_time && res.end_time) document.getElementById(rangeId).innerText = `${res.start_time} - ${res.end_time}`;
        }
        closeModal();
        loadSiteDatasets();
    })
...
function openUnifiedDownloadModal(){
    ...
    modalBody.innerHTML = `
        ...
        <div style="margin-top:12px;">
            ...
            <div style="margin-top:8px; font-size:12px; color:#666;">说明：将按此优先级合并同一时间点数据。</div>
        </div>
    ...
    .then(res=>{
        ...
        (res.details||[]).forEach(dev=>{
            ...
            (dev.results||[]).forEach(r=>{
                ...
                if(r.error || r.count<=0){ if(statusEl) statusEl.textContent="下载失败/无数据"; if(progress) progress.style.width="0%"; }
                else { if(statusEl) statusEl.textContent="下载完成"; if(progress) progress.style.width="100%";
                ...
                if(r.error) showToast(`站点 ${device} - ${r.type} 失败：${r.error}`,'error');
                else if(r.count===0) showToast(`站点 ${device} - ${r.type} 无数据`);
                else showToast(`站点 ${device} - ${r.type} 下载 ${r.count} 条`);
            });
        });

        const mergedList = res.merged?.results || res.merged || [];
        mergedList.forEach(m=>{
            ...
            if(!datasets.some(d=>d.path===rec.path && d.site===rec.site && d.sub_dir===rec.sub_dir)) datasets.push(rec);
        });
...
function renderDatasetTableMultiSite(fileList){
    ...
    if(!fileList?.length){ tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">暂无数据</td></tr>`; return; }

    fileList.forEach(item=>{
        const filePath = [item.sub_dir||'', item.file].filter(Boolean).join('/');
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td>${item.file}</td>
            <td>${item.site||'未知站点'}</td>
            <td>${item.site_name||item.site}</td>
            <td>${item.start_time} ~ ${item.end_time}</td>
            <td>${item.created_at}</td>
            <td>
                <button class="btn detail-btn" onclick="showDetails('${item.site}','${item.sub_dir||'train'}','${item.file}')">详情</button>
                <button class="btn delete-btn" onclick="deleteDataset('${encodeURIComponent(filePath)}','${item.site}')">删除</button>
            </td>
        `;
...
function showDetails(site, mode, file){
    ...
    .then(res=>res.json())
    .then(data=>{
        if(data.error){ modalBody.innerHTML=`<h3>${file} - ${site} 数据详情</h3><p style="color:red;">${data.error}</p>`; }
        else{
            ...
            rows.forEach(r=>{ tableHTML+=`<tr><td>${r.date||''}</td><td>${r.t2m??''}</td><td>${r.sp??''}</td><td>${r.rh??''}</td><td>${r.pwv??''}</td><td>${r.tp??''}</td></tr>`; });
...
function renderDatasetTable(page){
    ...
    const tbody = document.getElementById('datasetTableBody');
    tbody.innerHTML='';
    const pageData = datasets.slice((page-1)*pageSize, (page-1)*pageSize+pageSize);

    pageData.forEach(ds=>{
        const filePath = [ds.sub_dir||'', ds.file].filter(Boolean).join('/');
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${ds.file}</td><td>${ds.site||'未知站点'}</td><td>${ds.site_name||ds.site}</td><td>${ds.start_time} ~ ${ds.end_time}</td>
                        <td>${ds.created_at}</td>
                        <td>
                            <button class="btn detail-btn" onclick="showDetails('${ds.site}','${ds.sub_dir||'train'}','${ds.file}')">详情</button>
                            <button class="btn delete-btn" onclick="deleteDataset('${encodeURIComponent(filePath)}','${ds.site}')">删除</button>
                        </td>`;
        tbody.appendChild(tr);
    });
...
function loadStationList(){
    ...
    .then(data=>{
        ...
        dataList.innerHTML='';
        data.forEach(site=>{ const opt=document.createElement('option'); opt.value=site.site_id; dataList.appendChild(opt); });

        if(!stations.length && data.length){ stations.push(data[0].site_id); renderStationTags(); }
...
function loadSiteDatasets(){
    ...
    .then(r=>r.json())
    .then(res=>{
        (res.train||[]).forEach(d=>datasets.push({...d,sub_dir:'train',file:d.name,path:d.path}));
        (res.pred||[]).forEach(d=>datasets.push({...d,sub_dir:'pred',file:d.name,path:d.path}));
...
function loadDatasetForAnalysis(path,silent=false){ return; }
