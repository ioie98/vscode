    ...
    .then(res=> {
        ...
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            ...
            mergedList.forEach(m => {
                const rec = {
                    file: m.dataset || `dataset-${now.toISOString().replace(/[:.]/g,'-')}.csv`,
                    site: m.site_id,
                ...
                // push into datasets for current site if matches
...
function renderDatasetTableMultiSite(fileList){
    ...
    fileList.forEach(item => {
        const file = item.dataset || item.file || '';
        const siteId = item.site_id || item.site || '未知站点';       
        const siteName = item.site_name || '站点' + siteId;           
        const startTime = item.time_range?.split('~')[0]?.trim() || '';
        const endTime = item.time_range?.split('~')[1]?.trim() || '';
        const createdAt = item.created_at || new Date().toLocaleString();
...
function loadSiteDatasets() {
    ...
    fetch(`/get_datasets?site=${encodeURIComponent(site)}`)
        ...
        .then(res => {
            ...
            // 清空 datasets 数组并填充为统一格式以供分页（混合）
            datasets = [];

            // 先 train，再 pred - 但我们保留 sub_dir 字段以区分
            trainList.forEach(d => {
                datasets.push({
                    file: d.name,
                    site: d.site,
...
// 删除文件（路径格式: subdir/filename 或 filename）
