    ...
    .then(res=> {
        ...
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            ...
            mergedList.forEach(m => {
                // 构造新命名规则: 站点ID_起始日期_结束日期.csv
                const startDate = (m.time_range || '').split('~')[0]?.replace(/[-/]/g,'') || '';
                const endDate = (m.time_range || '').split('~')[1]?.replace(/[-/]/g,'') || '';
                const newFileName = `${m.site_id}_${startDate}_${endDate}.csv`;
                ...
                const rec = {
                    ...
                    path: m.dataset             // <- 后端实际文件名，用于删除/详情
                };
...
function renderDatasetTable(datasets) {
    ...
    const pageData = datasets.slice(startIndex, startIndex + pageSize);

    pageData.forEach(row => {
        const tr = document.createElement('tr');

        // 站点编号
        const tdSiteId = document.createElement('td');
        tdSiteId.textContent = row.site_id || '';
        tr.appendChild(tdSiteId);

        // 站点名称
        const tdSiteName = document.createElement('td');
        tdSiteName.textContent = row.site_name || '';
        tr.appendChild(tdSiteName);

        // 时间范围
        const tdRange = document.createElement('td');
        tdRange.textContent = row.time_range || '';
        tr.appendChild(tdRange);

        // 创建时间
        const tdCreated = document.createElement('td');
        tdCreated.textContent = row.created_time || '';
        tr.appendChild(tdCreated);

        // 合并文件名（原来可能是 undefined）
        const tdMerged = document.createElement('td');
        tdMerged.textContent = row.merged_file || '无合并文件';
        ...
        detailBtn.onclick = () => showDetails(row);
