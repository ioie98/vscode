    ...
    .then(res => {
        ...
        for(const dev of details){
            ...
            for(const r of dev.results || []){
                ...
                // --- 核心修改：按每次下载请求生成独立文件 ---
                const startStr = start.replace(/[-:\s]/g,'').slice(0,8);
                const endStr = end.replace(/[-:\s]/g,'').slice(0,8);
                const fileName = `${device}_${startStr}_${endStr}.csv`;
