    ...
    .then(res=> {
        ...
        if(res.merged && ((res.merged.results && Array.isArray(res.merged.results)) || Array.isArray(res.merged))) {
            ...
            mergedList.forEach(m => {
                ...
                let endTime = '';
                if(m.time_range){
                    const arr = m.time_range.split('~');
                    startTime = arr[0]?.trim() || '';
                    endTime = arr[1]?.trim() || '';
                    // 格式化为 YYYYMMDD
                    startTime = startTime.replace(/[-:\s]/g,'').slice(0,8);
                    endTime = endTime.replace(/[-:\s]/g,'').slice(0,8);
                }

                const newFileName = m.dataset || `${m.site_id || 'site'}_${startTime}_${endTime}.csv`;
