...
async function uploadDatasetToOSS(siteId, mode, filename){
    if(!siteId || !mode || !filename){
        showToast('缺少上传参数', 'error');
        return;
    }

    showToast(`上传 ${filename} 中...`, 'info'); // info 也用绿色即可

    try {
        const resp = await fetch('/upload_to_tos', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ site: siteId, mode: mode, filename: filename })
        });

        const data = await resp.json();
        if(data.status === 'success'){
            showToast(`上传成功: ${data.key}`, 'success');
        } else {
