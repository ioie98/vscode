...
function renderDatasetTableMultiSite(fileList){
    ...
    fileList.forEach(item => {
        ...
        const filePath = `${item.sub_dir}/${siteId}/${file}`;
        ...
        tr.innerHTML = `
            ...
            <td>${createdAt}</td>
            <td>
                <button class="btn delete-btn" onclick="deleteDataset('${filePath}','${siteId}')">删除</button>
...
function deleteDataset(filename, site) {
    console.log("删除请求 filename =", filename, "site =", site);
    ...
    fetch(`/delete_dataset/${filename}/${site}`, { method:'DELETE' })
        ...
        .then(res => {
            ...
            if (res.status === 'success' || res.message?.includes('不存在')) {
                showToast('删除成功');

                // 删除 datasets 中的记录（最稳写法）
                datasets = datasets.filter(d => {
                    const fullPath = `${d.sub_dir}/${d.site}/${d.file}`.replace(/\\/g, "/");
                    console.log("对比 fullPath =", fullPath);
                    return fullPath !== filename;
                });

                console.log("删除后的 datasets =", datasets);
...
function renderDatasetTable(page){
    ...
    pageData.forEach(ds => {
        ...
        const filePath = `${subDir}/${siteId}/${file}`;
...
function downloadDataset(siteId, subDir, file){
    ...
    window.location.href = "/download_dataset/" + filePath;
}

#
async function uploadDatasetToOSS(siteId, mode, filename){
    if(!siteId || !mode || !filename){
        alert('缺少上传参数');
        return;
    }

    // 假设你的 CSV 文件本地存储在 fileList 对象里
    const item = window.currentFileList?.find(f => f.file === filename && f.site === siteId && (f.sub_dir||'train') === mode);
    if(!item || !item.fileObj){
        alert('找不到文件对象，无法上传');
        return;
    }

    const formData = new FormData();
    formData.append('file', item.fileObj); // File 对象
    formData.append('site', siteId);
    formData.append('mode', mode);

    const statusDiv = document.getElementById('uploadStatus');
    statusDiv.textContent = `上传 ${filename} 中...`;

    try {
        const resp = await fetch('/upload_to_tos', {
            method: 'POST',
            body: formData
        });
        const data = await resp.json();
        if(data.status === 'success'){
            statusDiv.textContent = `上传成功: ${data.key}`;
        } else {
