...
function renderStationTags() {
    ...
    box.innerHTML = '';
    stations.forEach(site_id => {
        const option = Array.from(document.getElementById('stationList').options)
            .find(o => o.value === site_id);
        const name = option ? option.text : site_id;

        const span = document.createElement('span');
        span.className = 'station-tag';
        span.innerHTML = `${name} <span class="remove-tag" onclick="removeStation('${site_id}')">×</span>`;
...
function loadStationList() {
    fetch('/get_stations')
        ...
        .then(data => {
            ...
            dataList.innerHTML = ''; // 清空原有 option
            data.forEach(site => {
                const option = document.createElement('option');
                option.value = site.site_id;   // 输入框用 site_id
                option.text = site.site_name;  // 显示站点名
                dataList.appendChild(option);
            });

            // 页面加载时默认选第一个站点
            if (data.length > 0) input.value = data[0].site_id;
        })
        .catch(err => console.error('加载站点列表失败', err));
}
