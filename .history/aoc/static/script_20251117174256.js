...
function startUnifiedDownload(){
    ...
    .then(res=> {
        ...
        // 整合文件命名规则: 站点ID_起始_结束.csv
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            const mergedList = res.merged.results;
            mergedList.forEach(m => {
                const fileName = `${m.site_id}_${start.replace(/-/g,'').replace(/:/g,'')}_${end.replace(/-/g,'').replace(/:/g,'')}.csv`;
                const rec = {
                    file: fileName,
                    site: m.site_id,
                ...
                if(!datasets.some(d => d.file === rec.file && d.site === rec.site)){
                    datasets.push(rec);
                }
            });
...
function deleteDataset(filename, site){
    ...
    fetch(`/delete_dataset?site=${encodeURIComponent(siteVal)}&file=${filename}`)
    .then(r=>r.json())
    .then(res=>{
        if(res.success){
            showToast('删除成功');
...
function renderDatasetTable(page){
    ...
    const endIdx = Math.min(datasets.length, startIdx+pageSize);
    ...
    for(let i=startIdx;i<endIdx;i++){
        ...
        tr.innerHTML = `
            ...
            <td>
                <button class="btn detail-btn" onclick="showDetails('${item.site}','${item.sub_dir}','${item.file}')">详情</button>
...
window.onload = function(){
    loadStationList();
    document.getElementById('stationSelect')?.addEventListener('keydown', onStationInputKeydown);
};
