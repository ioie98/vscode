    ...
    .then(res=> {
        ...
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            ...
            mergedList.forEach(m => {
                const startStr = (m.time_range || '').split('~')[0]?.trim().slice(0,10).replace(/-/g,'') || '';
                const endStr = (m.time_range || '').split('~')[1]?.trim().slice(0,10).replace(/-/g,'') || '';
                const fileName = `${m.site_id}_${startStr}_${endStr}.csv`;
