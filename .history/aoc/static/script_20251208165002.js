...
async function uploadDatasetToOSS(siteId, mode, filename){
    if(!siteId || !mode || !filename){
        ...
        if(statusDiv) statusDiv.textContent = '缺少上传参数';
        return;
    }

    const statusDiv = document.getElementById('uploadStatus');
    if(statusDiv) statusDiv.textContent = `上传 ${filename} 中...`;
    ...
    try {
        ...
        if(data.status === 'success'){
            if(statusDiv) statusDiv.textContent = `上传成功: ${data.key}`;
        } else {
            if(statusDiv) statusDiv.textContent = `上传失败: ${data.error}`;
        }
    } catch(err){
        console.error(err);
        if(statusDiv) statusDiv.textContent = '上传请求失败，请检查网络或后端服务';
    }
}
