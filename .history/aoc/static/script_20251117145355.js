// static/script.js - 完整版 (替换你现有的 script.js)
// 说明：此脚本基于你之前的前端逻辑，仅对下载合并命名、mode 传参、列表分组等做必要调整。
...
// ---------- 批量下载（会把 mode/start_time/end_time 一并发给后端） ----------
function startUnifiedDownload(){
    ...
    const mode = modeEl ? modeEl.value : 'train'; // train / pred
    ...
    .then(res=> {
        ...
        for(const dev of details){
            ...
            for(const r of dev.results || []){
                ...
                if(r.error){
                    ...
                    if(statusEl) statusEl.textContent = "无数据";
                    if(progress) progress.style.width = "0%";
        ...
        if(res.merged && res.merged.results && Array.isArray(res.merged.results)){
            ...
            mergedList.forEach(m => {
                const rec = {
                    file: m.dataset.split('/').pop(),
                    site: m.site_id,
                    site_name: m.site_name || m.site_id,
                    sub_dir: mode, // train / pred
                    start_time: (m.time_range || '').split('~')[0]?.trim() || '',
                    end_time: (m.time_range || '').split('~')[1]?.trim() || '',
                    created_at: now.toLocaleString(),
                    path: `${m.site_id}/${mode}/${m.dataset.split('/').pop()}` // 完整路径
                };

                if(!datasets.some(d => d.file === rec.file && d.site === rec.site && d.sub_dir === rec.sub_dir)){
                    datasets.push(rec);
                }
            });
...
function renderDatasetTableMultiSite(fileList){
    ...
    if(!tbody) return;
    tbody.innerHTML = '';
    if(!fileList || fileList.length===0){ tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">暂无数据</td></tr>`; return; }
    ...
    fileList.forEach(item=>{
        ...
        const subDir = item.sub_dir || 'train';
        const filePath = [subDir, file].filter(Boolean).join('/');
