...
// 加载 MySQL 中 sb_jbxx.ID 作为下拉选项
function loadStationOptions(){
    fetch('/get_station_options')
    .then(res => res.json())
    .then(data => {
        const input = document.getElementById('stationSelect');
        const dataList = document.getElementById('stationList');
        ...
        // 默认显示第一个站点
        if(data.length>0 && stations.length===0){
            input.value = data[0];
        }
    }).catch(err => console.error('加载站点选项失败', err));
}
...
function addSelectedStations(){
    ...
    if(selected.length===0){ showToast('请至少选择一个站点','error'); return; }
