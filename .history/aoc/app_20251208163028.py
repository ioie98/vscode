...
ak = os.getenv('TOS_ACCESS_KEY', 'AKLTNzQ3YTk1NjQ4YmU4NDBmMDk0NTI2ZjlhN2YzODdkY2E')
sk = os.getenv('TOS_SECRET_KEY', 'T1RCbE9EYzFOalUzTXpFME5EaGxaR0ZsWkRFME5EVmtZVEkxTnpSaU5HTQ==')
...
def upload_to_tos():
    try:
        ...
        site = req.get('site')
        mode = req.get('mode')
        filename = req.get('filename')  # CSV 在服务器的名字

        if not site or not mode or not filename:
            return jsonify({'status':'error', 'error':'缺少参数'}), 400

        # 服务器上 CSV 文件路径
        server_dir = "./csv_output"  # 根据实际存放目录修改
        file_path = os.path.join(server_dir, mode, site, filename)
