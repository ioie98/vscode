...
# 数据上传接口（直接上传到 ClickHouse，CSV 时间为北京时间 -> 存为 UTC，并映射所有字段）
...
def upload_data():
    try:
        ...
        insert_df['data_time'] = insert_df['data_time'] - pd.Timedelta(hours=8)
            ...
            for target_field, candidates in col_map.items():
                if lc in [c.lower() for c in candidates]:
                    if target_field in ['rain_time','reset']:
                        ...
                        # 经纬度、device、data_type 不转数字
                        if target_field in ['lat','lon','device','data_type','data_time']:
                            insert_df[target_field] = df[col]
                        else:
        ...
        try:
            ...
            for row in res.result_rows:
                t = row[0]
                existing_times.add(t.strftime("%Y-%m-%d %H:%M:%S") if isinstance(t, datetime) else str(t))
        except Exception as e:
            existing_times = set()
        ...
        if not to_insert_df.empty:
            ...
            for _, row in to_insert_df.iterrows():
                rec = []
                for f in cfg['fields']:
                    rec.append(None if pd.isna(row.get(f)) else row[f])
