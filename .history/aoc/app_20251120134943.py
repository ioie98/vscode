...
def upload_data():
    try:
        ...
        for _, row in df.iterrows():
            ...
            if pd.isna(dt):
                continue
            dt_utc = dt - pd.Timedelta(hours=8)  # 北京时间 -> UTC
            ...
            if dtype == 'pwv':
                buffer_list.append({
                    ...
                    'lau': str(row.get('lau', '0')),
                    'pwv': safe_float(row.get('pwv', 0.0)),
                ...
                buffer_list.append({
                    'data_time': dt_utc,
                    'device': str(row.get('device', device)),
                    'ta': safe_float(row.get('ta', 0.0)),
                    'pa': safe_float(row.get('pa', 0.0)),
                    'ua': safe_float(row.get('ua', 0.0)),
                    'data_type': row.get('data_type', 'uploaded')
                })
            elif dtype == 'rain':
                buffer_list.append({
                    'data_time': dt_utc,
                    'device': str(row.get('device', device)),
                    'rainfall': safe_float(row.get('rainfall', 0.0)),
                    'data_type': row.get('data_type', 'uploaded'),
                    'rain_time': safe_int(row.get('rain_time', 0)),
                    'reset': safe_int(row.get('reset', 0))
                })
        ...
        buffer_list = [r for r in buffer_list if r['data_time'].strftime("%Y-%m-%d %H:%M:%S") not in existing_times]

        # --------------------- 3. 构建二维数组并插入 ---------------------
        records_list = [[r[f] for f in fields] for r in buffer_list]
        inserted_count = 0
        if records_list:
            try:
