    ...
    for site in sites:
        ...
        for f in os.listdir(mode_dir):
            ...
            if os.path.isfile(full) and f.lower().endswith('.csv'):
                # 排除已有合并文件（site_YYYYMMDD_YYYYMMDD.csv 或 site_YYMMDD_YYMMDD.csv）
                if re.match(rf'^{re.escape(site)}_\d{{8}}_\d{{8}}\.csv$', f, re.I) or re.match(rf'^{re.escape(site)}_\d{{6}}_\d{{6}}\.csv$', f, re.I):
                    continue
        ...
        dataset_name = f"{site}_{start_short}_{end_short}.csv"
...
def serve_tmp_data(filename, site):
    # 兼容前端传入 'train/xxx.csv' 或 'pred/xxx.csv'，site 提供站点 id
    try:
