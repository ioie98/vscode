...
def batch_download():
    try:
        ...
        if not types or not start_time or not end_time or not devices:
            return jsonify({'error': '参数缺失'}), 400
        ...
        for site in devices:
            site_detail = {'device': site, 'results': []}
            site_dir = os.path.join(TMP_DIR, site, mode)
            os.makedirs(site_dir, exist_ok=True)

            # 下载每种类型数据（后端已有下载逻辑，略）
            for t in types:
                # 这里假设 download_data 返回 dataframe 或列表
                try:
                    df = download_data(site, t, start_time, end_time, priority)
                    count = len(df)
                    # 保存原始单类型文件（可选）
                    # df.to_csv(os.path.join(site_dir, f"{t}_{site}.csv"), index=False)
                    site_detail['results'].append({'type': t, 'count': count})
                except Exception as e:
                    site_detail['results'].append({'type': t, 'error': str(e)})

            # 合并文件逻辑
            try:
                merged_df = merge_site_data(site, types, start_time, end_time, priority)
                # 命名规则改为：site_YYYYMMDD_YYYYMMDD.csv
                start_str = start_time.replace('-', '').replace(':','').replace('T','')[:8]  # YYYYMMDD
                end_str   = end_time.replace('-', '').replace(':','').replace('T','')[:8]    # YYYYMMDD
                merged_filename = f"{site}_{start_str}_{end_str}.csv"
                merged_path = os.path.join(site_dir, merged_filename)
                merged_df.to_csv(merged_path, index=False)

                merged_results.append({
                    'site_id': site,
                    'site_name': site,  # 可按需要替换为正式名称
                    'sub_dir': mode,
                    'dataset': merged_filename,
                    'time_range': f"{start_time} ~ {end_time}"
                })
            except Exception as e:
                site_detail['results'].append({'type': 'merged', 'error': str(e)})

            details.append(site_detail)

        return jsonify({'details': details, 'merged': {'results': merged_results}})
    except Exception as e:
