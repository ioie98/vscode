...
def batch_download():
    try:
        ...
        # 下载完成后自动合并
        try:
            merged = merge_datasets_inner(devices)
        except Exception as e:
...
# 接收站点列表并合并到 site 根目录（不使用子目录）
def merge_datasets_inner(sites):
    results = []
    for site in sites:
        ...
        # 收集 site 根目录和子目录里的所有非 dataset 文件（csv）
        csv_files = []
        # 根目录的 csv
        ...
        # 子目录里的 csv（例如原先的 time-range 子目录）
...
def merge_datasets():
    try:
        ...
        merged = merge_datasets_inner(selected_sites)
...
def get_datasets():
    ...
    for root, _, files in os.walk(site_dir):
        for file in files:
            # 支持 dataset_ 和 dataset- 开头
            if re.match(r'^dataset[-_]', file) and file.endswith('.csv'):
                ...
                datasets.append({
                    "sub_dir": os.path.dirname(rel) if os.path.dirname(rel) != '.' else '',
                    "name": file,
                    "site": site,
                    "site_name": f"站点{site}",
                    "start_time": start_time,
    ...
    # 按文件创建时间倒序
    datasets.sort(key=lambda x: os.path.getctime(os.path.join(site_dir, x['path'])), reverse=True)
