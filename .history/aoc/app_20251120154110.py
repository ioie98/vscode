...
def upload_data():
    try:
        ...
        alias_map = {
            ...
            'latitude': 'lau', 'lat': 'lau',

            # PWV
            'pwv': 'pwv',

            # HWS
            'temp': 'ta', 'temperature': 'ta',
            'pressure': 'pa',
            'humidity': 'ua', 'rh': 'ua',
        ...
        # -------------------- 识别时间列 --------------------
        time_col = next((c for c in df.columns if 'data_time' in c or 'time' in c or '日期' in c), None)
        if not time_col:
        ...
        for _, row in df.iterrows():
            ...
            if dtype == 'pwv':
                ...
                item = {
                    ...
                    'data_type': row.get('data_type', 'uploaded'),
                    'rain_time': to_int(row.get('rain_time')),
                    'reset': to_int(row.get('reset'))
                }
