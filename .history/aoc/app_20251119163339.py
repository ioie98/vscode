...
# 数据上传接口（直接上传到 ClickHouse，CSV 时间为北京时间 -> 存为 UTC，并自动映射表字段）
...
def upload_data():
    try:
        ...
        insert_df['data_time'] = insert_df['data_time'] - pd.Timedelta(hours=8)  # 转 UTC
        ...
        def is_new_row(ts):
            ...
            if pd.isna(ts_dt):
                return False
            return ts_dt.strftime("%Y-%m-%d %H:%M:%S") not in existing_times

        to_insert_df = insert_df[insert_df['data_time'].apply(is_new_row)].copy()

        # 确保 data_time 列都是 datetime
        to_insert_df['data_time'] = pd.to_datetime(to_insert_df['data_time'], errors='coerce')
        to_insert_df = to_insert_df.dropna(subset=['data_time'])
