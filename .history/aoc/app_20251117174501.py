    ...
    try:
        ...
        if dtype in TABLE_CONFIG:
            try:
                ...
                # data_type 列：如果文件提供则使用，否则标记为 'uploaded'
                ...
                if insert_df.empty:
                    ...
                    # 去重：查数据库在该 device 与时间范围内已有的时间，排除已存在的时间
...
# 批量下载接口（自动整合） - 支持 mode 参数 (train/pred)
...
def batch_download():
    try:
        ...
        if not devices:
            return jsonify({'error':'缺少站点列表'})

        details = []
        merged_info = {'results':[]}

        for dev in devices:
            device_dir = os.path.join(TMP_DIR, dev, mode)
            if not os.path.exists(device_dir):
                details.append({'device':dev,'results':[{'type':t,'error':'文件夹不存在','count':0} for t in types]})
                continue
            ...
            for t in types:
                file_path = os.path.join(device_dir, f"{t}.csv")
                if not os.path.exists(file_path):
                    dev_results.append({'type': t, 'error':'文件不存在', 'count':0})
                    continue

                with open(file_path,'r',encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    rows = [r for r in reader if start_time <= r.get('time','') <= end_time]
                    dev_results.append({'type':t,'count':len(rows)})
                    merged_rows.append({'type':t,'rows':rows})
            ...
            for p in priority:
                for item in merged_rows:
                    ...
                    for row in item['rows']:
                        t = row['time']
                        merged_data[t].update(row)

            merged_list = sorted(merged_data.values(), key=lambda x:x['time']) if merged_data else []

            merged_file_name = ""
            if merged_list:
                start_fmt = start_time.replace('-','')
                end_fmt = end_time.replace('-','')
                merged_file_name = f"{dev}_{start_fmt}_{end_fmt}.csv"
                merged_file_path = os.path.join(device_dir, merged_file_name)
                with open(merged_file_path,'w',newline='',encoding='utf-8') as f:
                    writer = csv.DictWriter(f, fieldnames=merged_list[0].keys())
                    writer.writeheader()
                    writer.writerows(merged_list)
                dev_results.append({'type':'merged','file':merged_file_name,'count':len(merged_list)})
                merged_info['results'].append({'site_id':dev,'dataset':merged_file_name})

            details.append({'device':dev,'results':dev_results,'merged':{'results':[{'site_id':dev,'dataset':merged_file_name}] if merged_file_name else []}})

        return jsonify({'details': details, 'merged': merged_info})
    except Exception as e:
        return jsonify({'error': str(e)})


# 接收站点列表并合并到 site/<mode> 目录（只保存 dataset 文件，不保留原始 pwv/weather/rain）
def merge_datasets_inner(sites, mode='train'):
    ...
    for site in sites:
        ...
        # 收集 site 根目录和子目录里的所有 csv 文件（我们只合并非 dataset 文件）
        csv_files = []
        # 根目录的 csv（例如由 _download_single_type_for_device 生成）
        ...
        # 也检查任何子目录（历史结构），但我们不会把 dataset- 开头的文件当作输入
        ...
        if not csv_files:
            # 没有可合并的 CSV，则尝试查找已有 dataset 文件（可能之前手动放入）
            # 把现有 dataset-*.csv 列表返回，但不再生成新的合并
            existing = []
            # target mode dir
        ...
        # 合并（按优先级以 combine_first，顺序为文件出现顺序）
        ...
        # 生成 dataset 文件名并写到 site/<mode> 目录
        ...
        # 删除原始 csv 文（只删除用于合并的中间 csv，不删除 dataset- 开头文件）
        try:
            for fpath in csv_files:
                try:
...
# 获取文件行（详情预览） - 前端使用 site + mode + file
...
# 临时文件访问（预览） & 下载 & 删除（保留，但提醒：原来接口路径结构保持）
