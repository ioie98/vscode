...
def upload_data():
    try:
        ...
        # 页面显示时间（北京时间）
        if not df.empty:
            start_time = df['data_time'].min().strftime('%Y/%m/%d %H:%M:%S')
            end_time = df['data_time'].max().strftime('%Y/%m/%d %H:%M:%S')
        ...
        # pwv 表补充 lon / lau
        if dtype == 'pwv':
            if 'lon' not in insert_df.columns:
                insert_df['lon'] = request.form.get('lon') or 0.0
            if 'lau' not in insert_df.columns:
                insert_df['lau'] = request.form.get('lau') or 0.0
        ...
        to_insert_df = insert_df[~insert_df['data_time'].apply(lambda x: x.strftime("%Y-%m-%d %H:%M:%S") in existing_times)].copy()
