...
def upload_data():
    try:
        ...
        alias_map = {
            'longitude':'lon','lng':'lon','latitude':'lau','lat':'lau',
            'temp':'ta','temperature':'ta','pressure':'pa','humidity':'ua','rh':'ua',
            'tp_15min':'rainfall','time':'data_time','date':'data_time'
        }
        df.rename(columns={c: alias_map.get(c, c) for c in df.columns}, inplace=True)

        time_col = None
        for c in df.columns:
            if c in ['data_time'] or '时间' in c:
                time_col = c
                break
        if time_col is None:
        ...
        for _, row in df.iterrows():
            dt = pd.to_datetime(row.get(time_col, None), errors='coerce')
            ...
            if dtype == 'pwv':
                item = {
                    ...
                    'lau': to_float_or_none(row.get('lau')),
        ...
        return jsonify({'error': f'上传失败: {str(e)}','trace': traceback.format_exc()}),500
