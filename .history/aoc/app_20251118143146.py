...
def batch_download():
    try:
        ...
        devices = payload.get('devices') or []  # 站点列表
        start_time = payload.get('start_time')
        end_time = payload.get('end_time')
        mode = payload.get('mode', 'train')  # train 或 pred

        results = []

        for site_id in devices:
            # 对应模式文件夹
            site_dir = os.path.join(TMP_DIR, site_id, mode)
            os.makedirs(site_dir, exist_ok=True)

            # 模拟下载：生成几个 CSV 文件（真实环境替换成下载逻辑）
            for ftype in ['pwv', 'hws', 'temp']:
                fname = f"{ftype}_{start_time}_{end_time}.csv"
                fpath = os.path.join(site_dir, fname)
                pd.DataFrame({'time': [], 'value': []}).to_csv(fpath, index=False)

            # 合并 CSV 文件
            merged_filename = f"{site_id}_{start_time}_{end_time}.csv"
            merged_path = os.path.join(site_dir, merged_filename)

            all_files = [
                os.path.join(site_dir, f) for f in os.listdir(site_dir)
                if f.endswith('.csv') and f != merged_filename
            ]
            if all_files:
            ...
            results.append({'site_id': site_id, 'merged_file': merged_filename})
    ...
    for site in sites:
        ...
        if not csv_files:
            ...
            if os.path.exists(mode_dir):
                # 这里调整：同时支持旧的 dataset- 前缀 和 新的 站点ID_YYYYMMDD_YYYYMMDD.csv 格式
                for fn in os.listdir(mode_dir):
                    if fn.lower().endswith('.csv') and (re.match(r'^dataset[-_]', fn, re.I) or re.match(r'^[A-Za-z0-9]+_\d{8}_\d{8}\.csv$', fn)):
        ...
        # 生成合并后文件名： 站点ID_起始日期_结束日期.csv (YYYYMMDD)
        try:
            ...
            if pd.notna(first_date) and pd.notna(last_date):
                start_name = first_date.strftime('%Y%m%d')
                end_name = last_date.strftime('%Y%m%d')
            else:
                # 回退到当前日期（防止空）
                now = datetime.now()
                start_name = now.strftime('%Y%m%d')
                end_name = now.strftime('%Y%m%d')
        except Exception:
            now = datetime.now()
            start_name = now.strftime('%Y%m%d')
            end_name = now.strftime('%Y%m%d')
    ...
    def list_mode_files(mode):
        ...
        if os.path.exists(mode_dir):
            for file in os.listdir(mode_dir):
                # 兼容新的命名规则 site_YYYYMMDD_YYYYMMDD.csv 和 旧的 dataset-*.csv
