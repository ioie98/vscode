...
def get_datasets():
    ...
    if not site:
        ...
        for s in os.listdir(TMP_DIR):
            if os.path.isdir(os.path.join(TMP_DIR, s)):
                result.append({"site_id": s})
        return jsonify(result)
    else:
        # 返回指定站点 train/pred
        site_path = os.path.join(TMP_DIR, site)
        res = {"train": [], "pred": []}
        for sub_dir in ["train", "pred"]:
            dir_path = os.path.join(site_path, sub_dir)
            ...
            for f in os.listdir(dir_path):
                if f.endswith(".csv"):
                    res[sub_dir].append({
                        ...
                        "start_time": "",
                        "end_time": "",
                        "created_at": datetime.datetime.fromtimestamp(os.path.getctime(os.path.join(dir_path,f))).strftime("%Y-%m-%d %H:%M:%S"),
                        "path": f
                    })
        return jsonify(res)
