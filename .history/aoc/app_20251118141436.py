...
# 接收站点列表并合并到 site/<mode> 目录，命名规则改为：siteid_开始日期_结束日期.csv
def merge_datasets_inner(sites, mode='train'):
    ...
    for site in sites:
        ...
        # 生成 dataset 文件名：siteid_开始日期_结束日期.csv
        first_date = pd.to_datetime(merged['date'], errors='coerce').min()
        last_date = pd.to_datetime(merged['date'], errors='coerce').max()
        start_str = first_date.strftime('%Y%m%d') if pd.notna(first_date) else 'unknown'
        end_str = last_date.strftime('%Y%m%d') if pd.notna(last_date) else 'unknown'
