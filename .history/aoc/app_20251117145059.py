...
def batch_download():
    try:
        ...
        mode = data.get('mode', 'train')  # train / pred

        if not devices or not types or not start_time or not end_time:
            return jsonify({'error': '参数缺失'}), 400
        ...
        for device in devices:
            ...
            # 确保目录存在
            save_dir = os.path.join(TMP_DIR, device, mode)
            os.makedirs(save_dir, exist_ok=True)

            for t in types:
                # 这里假设根据 type、device、start/end_time 拉取数据
                # 示例使用空 DataFrame
                df = pd.DataFrame({
                ...
                if df.empty:
                    dev_results.append({'type': t, 'count': 0})
                    continue

                # 文件名命名
                dataset_name = f"{device}_{start_time.replace('-','').replace(':','').replace('T','_')}_{end_time.replace('-','').replace(':','').replace('T','_')}.csv"
                file_path = os.path.join(save_dir, dataset_name)

                # 保存 CSV
                df.to_csv(file_path, index=False)

                dev_results.append({'type': t, 'count': len(df)})

                # 添加 merged 记录，只返回合并文件名
            ...
            details.append({'device': device, 'results': dev_results})
