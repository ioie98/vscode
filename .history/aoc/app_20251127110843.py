...
def get_stations():
    conn = pymysql.connect(host='115.190.81.184', user='user', password='pwd', db='dbname', charset='utf8')
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cur:
            cur.execute("SELECT site_id, site_name FROM stations ORDER BY site_name")
            result = cur.fetchall()
