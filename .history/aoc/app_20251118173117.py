...
MYSQL_CONFIG = {
    ...
    'database': 'your_database_name',  # 替换成实际库名
    'charset': 'utf8mb4'
}
# ---------------- MySQL 查询站点名称 ----------------
def get_site_name_from_db(site_id):
    try:
        conn = pymysql.connect(**MYSQL_CONFIG)
        with conn.cursor() as cursor:
            sql = "SELECT NAME FROM sb_jbxx WHERE ID=%s LIMIT 1"
            cursor.execute(sql, (site_id,))
            row = cursor.fetchone()
            if row:
                return row[0]
    except Exception as e:
        print(f"MySQL 查询站点名称失败: {e}")
    finally:
        try: conn.close()
        except: pass
    return site_id  # 查询失败或没找到则返回原站点编号
...
def merge_datasets_inner(sites, mode='train'):
    ...
    for site in sites:
        ...
        # 只扫描 CSV 文件（非 dataset-* 开头）
        csv_files = [os.path.join(mode_dir, f) for f in os.listdir(mode_dir)
                     if os.path.isfile(os.path.join(mode_dir, f)) 
                     and f.lower().endswith('.csv') 
                     and not re.match(r'^dataset[-_]', f, re.I)]
        ...
        # 查询数据库获取站点名称
        site_name = get_site_name_from_db(site)
