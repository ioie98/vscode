...
def merge_datasets_inner(sites, mode='train'):
    ...
    for site in sites:
        ...
        # 旧目录: tmp_data/site/mode
        old_mode_dir = os.path.join(site_path, mode)
        if not os.path.exists(old_mode_dir):
            results.append({'site_id': site, 'error': f'{mode} 目录不存在'})
            continue

        # 读取 CSV（仅本次下载的）
        csv_files = []
        for f in os.listdir(old_mode_dir):
            full = os.path.join(old_mode_dir, f)
        ...
        # === 改成 YYMMDD ===
        start_short = start_name[2:]
        end_short = end_name[2:]

        filename = f"{site}_{start_short}_{end_short}.csv"
        save_path = os.path.join(mode_dir, filename)

        try:
            merged.to_csv(save_path, index=False, encoding='utf-8-sig')
        except Exception as e:
            results.append({'site_id': site, 'error': f'保存失败: {str(e)}'})
            continue
