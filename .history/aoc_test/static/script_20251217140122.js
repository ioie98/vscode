...
function startUnifiedDownload() {
    ...
    // 如果输入框有值，且是 "名称 ID" 格式，提取 ID
    if (inputVal) {
        const parts = inputVal.split(/\s+/);
        inputVal = parts[parts.length - 1]; // 取最后一部分作为 ID
    }

    // 如果列表为空但输入框有值，则下载输入框的站点
    ...
    // 3. 确定 UI 应该显示哪个站点的进度
    // 规则：优先显示输入框里的站点；如果输入框为空，则显示列表中的第一个站点
    ...
    .then(res => {
        ...
        (res.details || []).forEach(dev => {
            // 关键修复：判断当前返回的设备是否是 UI 上正在展示的那个设备
            // 如果 dev.device 等于 currentUiSite，才更新表格上的进度条
...
function showConfirmModal(message, onConfirm) {
    ...
    // 渲染确认框 HTML，样式模仿简洁的卡片/Toast风格
    modalBody.innerHTML = `
        <div style="text-align: center; padding: 10px 0;">
            <div style="font-size: 16px; margin-bottom: 25px; color: #333;">${message}</div>
            <div style="display: flex; justify-content: center; gap: 20px;">
                <button class="btn" id="confirmBtnCancel" style="background-color: #f5f5f5; color: #666; border: 1px solid #ddd;">取消</button>
                <button class="btn delete-btn" id="confirmBtnOk">确定</button>
            </div>
        </div>
    `;
