...
function startUpload(type) {
    ...
    // 提取纯ID，防止 "名称 ID" 造成上传匹配问题（如果后端需要纯ID）
    // const realStationId = stationId.split(/\s+/).pop();
    // formData.set('device', realStationId);
...
function startUnifiedDownload() {
    ...
    // 3. 确定当前UI应该显示哪个站点的进度 (优先输入框，否则列表第一个)
    ...
    .then(res => {
        ...
        (res.details || []).forEach(dev => {
            // 只有当返回的设备 ID 与当前 UI 显示的站点 ID 一致时，才更新进度条
...
function showConfirmModal(message, onConfirm) {
    const modalBg = document.getElementById('modalBg');
    const modalBody = document.getElementById('modalBody');
    const modalFooter = document.getElementById('modalFooter'); // HTML中需添加 id="modalFooter"
        ...
        <div style="text-align: center; padding: 20px 10px;">
            ...
            <div style="display: flex; justify-content: center; gap: 15px;">
                ...
                <button class="btn" onclick="closeModal()" 
                    style="background-color: #f0f2f5; color: #555; border: 1px solid #dcdfe6; min-width:80px;">
                    取消
                </button>
                <!-- 确定按钮 -->
