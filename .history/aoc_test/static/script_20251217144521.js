...
function closeModal() {
    ...
    // 关键：恢复底部通用关闭按钮的显示，防止影响下一次打开其他弹窗
...
function openUploadModal(type) {
    ensureModalFooterVisible(); // 显示通用关闭按钮
...
function showConfirmModal(message, onConfirm) {
    const modalBg = document.getElementById('modalBg');
    const modalBody = document.getElementById('modalBody');
    const modalFooter = document.getElementById('modalFooter'); // 对应HTML中新增的id
...
async function loadOptions() {
    ...
    try {
        ...
        // 【修改】：删除了这里“如果 stations 为空则自动加载第一条数据”的逻辑
        // 现在的逻辑：初始页面不选择任何站点，表格为空
