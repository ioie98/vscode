    ...
    for mode in ['train', 'pred']:
        ...
        if os.path.exists(path):
            for f in os.listdir(path):
                ...
                count = 0
                try:
                    df = pd.read_csv(full_p, usecols=[0]) # 优化读取
                    count = len(df)
                except: 
                    count = 0
...
from urllib.parse import unquote, quote  # 确保引入了 quote
...
def upload_to_tos_route():
    ...
    try:
        ...
        # ---------------- 修改重点开始 ----------------
        # 1. 打印一下看看真实的 site 到底是什么，我猜它里面可能有中文
        print(f"DEBUG: site={site}, fname={fname}")

        # 2. 对 site 和 fname 都进行 URL 编码
        # 这样即使 site 是 "武汉站"，也会变成 "%E6%AD..."，不会报错
        safe_site = quote(str(site))
        safe_fname = quote(str(fname))

        # 3. 拼接 Key
        key = f"{mode}/{safe_site}/quantitative/{safe_fname}"
        # ---------------- 修改重点结束 ----------------
        ...
        # 回调业务接口参数 (这里也要用 safe_site，或者根据业务需求看是否允许中文)
        # 通常 URL 里的路径都建议编码
        file_abs_path = f"tos://{TOS_CFG['bucket']}/{mode}/{safe_site}/quantitative"
        
        payload = {
            "file_abs_path": file_abs_path,
            "file_name": fname # 告诉业务系统原始文件名
        }
        upload_api_result = {}
