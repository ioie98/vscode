...
# ClickHouse 数据库配置 (用于存储大规模气象/PWV数据)
CLICKHOUSE_CONFIG = {
...
def get_file_absolute_path(filename, site='', mode=''):
    ...
    # 1. 尝试直接拼接完整路径 (如 tmp_data/train/b100/xxx.csv)
    if mode and site:
    ...
    for source in priority:
        ...
        try:
            ...
            for row in [dict(zip(res.column_names, r)) for r in res.result_rows]:
                ...
                # 如果该时间点还没数据，则填入 (注意：这里实现了优先级逻辑，因为外层循环是按优先级来的吗？)
                # *修正理解*：如果 merged_data 中没有 key，则插入。意味着 priority 列表应该也是优先级高的在前，这里代码是 if t_str not in merged_data，所以高优先级的先占位。
...
def merge_datasets_inner(sites, mode, priority=None):
    ...
    for site in sites:
        ...
        # 查找该目录下本次下载生成的零散 CSV (排除掉已经是合并结果的文件)
        csv_files = [
...
def get_datasets_route():
    """
    获取数据集列表
    如果没传 site，返回所有有数据的站点列表
    如果传了 site，返回该站点下的所有合并好的 CSV 文件
    """
        ...
        if os.path.exists(path):
            for f in os.listdir(path):
                ...
                try:
                    ...
                    time_col = next((c for c in df.columns if c.lower() in ['date', 'time', 'data_time']), None)
                    t_range = (df[time_col].min(), df[time_col].max()) if time_col else (None, None)
                except: t_range = (None, None)
                
                resp[mode].append({
                    "name": f, "site": target_site, "site_name": get_site_name(target_site),
                    "start_time": str(t_range[0]), "end_time": str(t_range[1]),
...
def upload_to_tos_route():
    ...
    try:
        ...
        if not os.path.exists(fpath): return jsonify({'status':'error', 'error':'文件不存在'}), 400
        ...
        try:
            requests.post(cb_url, json={
                "file_abs_path": f"tos://{TOS_CFG['bucket']}/{key}", 
                "file_name": fname
            }, timeout=5)
        except: pass
