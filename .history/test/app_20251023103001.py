...
def merge_datasets():
    try:
        ...
        for site in site_dirs:
            ...
            # 获取下载的 csv 文件（pwv, weather, rain）而不是已有 dataset_ 合并文件
            csv_files = [os.path.join(site_path, f) for f in os.listdir(site_path) 
...
def get_datasets():
    ...
    for site in os.listdir(TMP_DIR):
        ...
        if os.path.isdir(site_path):
            ...
            # ✅ 改动：返回所有 dataset_*.csv 并排序
            merged_files = sorted([f for f in files if f.startswith('dataset_') and f.endswith('.csv')])
