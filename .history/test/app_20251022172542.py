...
def tmp_data_preview(filename, site):
    ...
    try:
        ...
        clean_df = pd.DataFrame({
            'date': df.get('date', pd.Series([None]*len(df))),
            't2m': df.get('t2m', pd.Series([None]*len(df))),
            'sp': df.get('sp', pd.Series([None]*len(df))),
            'rh': df.get('rh', pd.Series([None]*len(df))),
            'pwv': df.get('pwv', pd.Series([None]*len(df))),
            'tp': df.get('tp', pd.Series([None]*len(df)))
        })
