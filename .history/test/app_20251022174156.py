...
def merge_datasets():
    try:
        ...
        # ✅ 保存到 tmp_data/merged 文件夹
        merged_dir = os.path.join(TMP_DIR, "merged")
        os.makedirs(merged_dir, exist_ok=True)
        existing = [f for f in os.listdir(merged_dir) if f.startswith('dataset_')]
        dataset_name = f"dataset_{len(existing)+1}.csv"
        merged_path = os.path.join(merged_dir, dataset_name)
        ...
        return jsonify({
            ...
            'dataset': f"merged/{dataset_name}",  # 返回带文件夹路径
