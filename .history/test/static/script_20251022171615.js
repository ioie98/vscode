...
function loadSiteDatasets() {
    ...
    if (!siteId) {
        tbody.innerHTML = "<tr><td colspan='6'>请选择站点编号以加载数据</td></tr>";
        return;
    }
    ...
    fetch('/get_datasets')
        ...
        .then(allData => {
            ...
            if (!site || site.merged.length === 0) {
                tbody.innerHTML = "<tr><td colspan='6'>该站点没有合并数据</td></tr>";
                return;
            }
                    ...
                    .then(data => {
                        ...
                        tr.innerHTML = `
                          ...
                          <td>
                              ...
                              <button class="btn red" onclick="deleteDataset('${site.site_id}','${file}')">删除</button>
                          </td>
                        `;
...
// ---------- 删除 ----------
function deleteDataset(site, filename){
    fetch(`/delete_dataset/${site}/${filename}`, { method:'DELETE' })
        .then(res=>res.json())
        .then(res=>{
            if(res.success){
                showToast('删除成功');
