...
function loadSiteDatasets(){
    ...
    .then(res=>res.json())
    .then(allData=>{
        const site = allData.find(s=>s.site_id===stationId);
        if(!site || !site.merged || site.merged.length===0){
            tbody.innerHTML = `<tr><td colspan="6">该站点暂无合并数据</td></tr>`;
            return;
        }
            ...
            fetch(`/tmp_data_preview/${file}/${site.site_id}`)
              ...
              .then(data=>{
                  ...
                  tr.innerHTML = `
                      <td>${file}</td>
                      <td>${site.site_id}</td>
                      <td>站点${site.site_id}</td>
                      <td>${timeRange}</td>
                      <td>${now.toLocaleString()}</td>
                      <td>
                          <button class="btn" onclick="showDetails('${file}','${site.site_id}')">详情</button>
                          <button class="btn red" onclick="deleteDataset('${site.site_id}')">删除</button>
                      </td>
                  `;
...
// ---------- 删除 ----------
function deleteDataset(site){
    fetch(`/delete_dataset/${site}`, { method:'DELETE' })
        .then(res=>res.json())
        .then(res=>{
            if(res.success){
                showToast('删除成功');
