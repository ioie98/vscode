...
function startUpload(type) {
    ...
    const device = stationSelect.value || "B91";
...
function startDownload(type){
    ...
    const stationSelect = document.getElementById('stationSelect');
    const device = stationSelect.value || "B91";  // 使用顶部选择框
    ...
    fetch('/download_data', {
        ...
        body: JSON.stringify({ type, start_time: start, end_time: end, device })
    })
    .then(res=>res.json())
    .then(res=>{
        if(res.error){
