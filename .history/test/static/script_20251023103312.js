...
function loadStationList(){
    ...
    .then(data=>{
        ...
        for(const site of data){
            ...
            opt.textContent = `站点${site.site_id}`;
        ...
        // 选择站点变化时刷新
        select.onchange = ()=>loadSiteDatasets();
    });
}
...
function renderDatasetPage() {
    ...
    pageData.forEach(file => {
        fetch(`/tmp_data_preview/${file}/${document.getElementById('stationSelect').value}`)
            .then(res => res.json())
            .then(data => {
                let timeRange = "未知";
                ...
                tr.innerHTML = `
                  <td>${file}</td>
                  <td>${document.getElementById('stationSelect').value}</td>
                  <td>${document.getElementById('stationSelect').value}</td>
                  <td>${timeRange}</td>
                  <td>${now.toLocaleString()}</td>
                  <td>
                      <button class="btn" onclick="showDetails('${file}', '${document.getElementById('stationSelect').value}')">详情</button>
                      <button class="btn red" onclick="deleteDataset('${file}', '${document.getElementById('stationSelect').value}')">删除</button>
                  </td>
                `;
...
// ---------- 分页按钮 ----------
function prevPage() {
    if (currentPage > 1) { currentPage--; renderDatasetPage(); }
}
function nextPage() {
    const totalPages = Math.ceil(datasets.length / pageSize);
    if (currentPage < totalPages) { currentPage++; renderDatasetPage(); }
}
