...
function loadSiteDatasets() {
    ...
    fetch('/get_datasets')
        ...
        .then(allData => {
            ...
            let mergedFiles = site.merged || [];
            // 按时间排序，最新在前
            mergedFiles.sort((a, b) => {
                const ta = a.match(/\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}/);
                const tb = b.match(/\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}/);
                const dateA = ta ? new Date(ta[0].replace('_', ' ')) : new Date(0);
                const dateB = tb ? new Date(tb[0].replace('_', ' ')) : new Date(0);
                return dateB - dateA;
            });

            // 用 Promise.all 确保异步 fetch 完全处理
            const promises = mergedFiles.map(file => 
...
window.onload = ()=>{
    ...
    // ✅ 当选择站点时自动刷新整合结果
    const stationSelect = document.getElementById('stationSelect');
    stationSelect.onchange = loadSiteDatasets;
};
