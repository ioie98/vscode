...
function startUpload(type) {
    ...
    fetch('/upload_data', { method: 'POST', body: formData })
      ...
      .then(res => {
          ...
          closeModal();
          loadExistingDatasets(); // 上传完成刷新已有数据
      }).catch(err=>{
          row.textContent = "上传失败";
...
function startDownload(type){
    ...
    .then(res=>{
        ...
        closeModal();
        loadExistingDatasets(); // 下载完成刷新已有数据
    })
    .catch(err=>{
        statusEl.textContent = "下载失败";
        progress.style.width = "0%";
    ...
    for(const ds of pageData){
        ...
        tr.innerHTML = `
            ...
            <td>
                ...
                <button class="btn red" onclick="deleteDataset('${ds.stationId}')">删除</button>
            </td>`;
        ...
        .then(res=>{
            if(res.success){
                for(let i=datasets.length-1;i>=0;i--){
                    if(datasets[i].stationId===site) datasets.splice(i,1);
                }
                renderDatasetTable();
                showToast('删除成功');
